...
class PFLocaliser(PFLocaliserBase):
	...
	def update_particle_cloud(self, scan):
		...
		for particle in newParticleCloud.poses:
			newPose = Pose()
			newPose.position.x = random.gauss(particle.position.x,self.ODOM_DRIFT_NOISE)
			newPose.position.y = random.gauss(particle.position.y,self.ODOM_TRANSLATION_NOISE)
